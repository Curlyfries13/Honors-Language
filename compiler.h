...
struct FunctionNode
{
	...
	//char** aliasList;
	int argumentSize;
	struct ValueNode** varTable;
	struct StatementNode* body;
	struct StatementNode* link;
};

struct ArrayNode
{
	int tableSize;
	struct ValueNode** table;
};

// if we access an array we expect a expression
struct VarAccess
{
	bool isArrayAccess;
	union{
		struct ArrayNode* varArray;
...
struct FunctionStatement{
	...
	struct ValueNode** arguments;
	struct StatementNode* link;
};

//if op is 0 then only the LHS matters
struct Expr
{
	struct Term* LHS;
	int op;
...
struct Factor
{
	// 0 for expr, NUM for num, VAR for variable
	int type;
	union{
		struct Expr* expr;
		int num;
		struct VarAccess* var;
	};
};
...
struct IfStatement
{
	struct Expr* condition_operand1;
    struct Expr* condition_operand2;
...
struct StatementNode
{
    ...
    union
    {
		...
		struct FunctionStatement* function_call;
    };
...
struct ValueNode* accessValueNode(VarAccess* var);
int evaluateVar(VarAccess* var);
int evaluateExpr(Expr* expr);
int evaluateFactor(Factor* fac);
int  getToken();
void ungetToken();
